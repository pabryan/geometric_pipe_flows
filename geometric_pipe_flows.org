...
We are primarily interested in the equations of continuum mechanics constrained to an open embedded submanifold $M \overset{\phi}{\to} \RR^3$ with piecewise smooth boundary.  For instance $M$ could by a pipe or channel, a lake, a closed container, a star or a galaxy.  Often in this situation, the global coordinates are not appropriate for the problem at hand and so we work intrinsically on $M$.  For instance, we might be interested in the flow along the main axis of a pipe.  Let us then apply some Riemannian geometry to derive the balance laws in the Riemannian setting.  See the appendix for a review of the Riemannian geometry needed.
...
This Gauss map is Riemannian if and only if $\{E_i\}$ is an orthonormal frame, i.e. $\metric(E_i, E_j) = \delta_{ij}$, the Kronecker-delta function.
#+END_eg
...
There still remain infinitely many inequivalent Riemannian Gauss maps, to narrow the possibilities down to only one choice, inspired by the Euclidean case, we use another metric property.

#+BEGIN_prop
Let $U\subset M^n$ be a connected open set on which $\tangentbundle$ is trivial. Then there exists a unique Gauss map $\gaussmap{F}_U: \tangentbundle[U] \to U \times \RR^{n}$ ($n = \text{dim}(M)$) such the frame $\{F_U^{-1}(e_i)\}$ is a parallel (with respect to the Levi-Civita connection), orthonormal field. Here $e_i$ denotes the section of $U \times \RR^n$, $x \mapsto (x, (0, \dotsc, 0, 1, 0, \dotsc, 0)$ with the $1$ in the $i$'th position.

Moreover, if $\tangentbundle$ is trivial over the open set $V$ also, then $F_U = F_V$ on $U\intersect V$.
#+END_prop

Thus the proposition says that up to equivalence, there is a unique local Riemannian Gauss map over $U$ determined by the parallel orthonormal frames over $U$.

#+BEGIN_remark
The reader should be aware that just because $F_U$ agrees with $F_V$ on the overlap, we cannot in general "patch" together $\{F_{U_{\alpha}}\}$ with $\{U_{\alpha}\}$ an open cover of $M$ to obtain a Gauss map $F$ on all of $M$ with $m=n$. There are cohomological obstructions to such "patching".
#+END_remark

#+BEGIN_proof
The existence of $F_U$ is clear, since it is equivalent to the statement that over $U$ there exists a parallel orthonormal frame. To obtain such a frame, first produce by Gram-Schmidt an orthonormal basis at a point $x\in U$. Then since $\tangentbundle[U]$ is trivial, this basis may be parallel translated over $U$ to obtain a frame. By construction, the frame is parallel and since parallel translation is an isometry, it is also orthonormal.

To prove uniqueness, suppose we have two Gauss maps
\[
F_U, G_U: U\times \RR^n \to U\times \RR^n
\]
such that $\{F_i = F^{-1}(e_i)\}$ and $\{G_i = G^{-1}(e_i)\}$ are parallel orthonormal frames. In other words $F_U,G_U$ are isometries $(U \times \RR^n, \metric) \to (U \times \RR^n, \delta_{ij})$.

Then there exists a smooth map $\sigma: U \to GL_n(\RR)$ such that for each $x \in U$, writing $\sigma_x = \sigma(x)$ we have
$$
\{F_i\} = \{\sigma_x(G_i)\} \quad \text{and} \quad \metric(\sigma_x(G_i), \sigma_x(G_j)) = \metric(G_i, G_j).
$$
We need to show that $\sigma$ is constant. By assumption,
$$
\conx_X F_i = \conx_X G_i = 0
$$
for each $i = 1,\dotsc, n$ and any vector field $X$ on $U$ where $\conx$ is the Levi-Civita connection of $\metric$. For convenience, since $U$ is connected and by continuity, reordering if necessary, we may assume that $F_i = \sigma(G_i)$ for every $x\in U$.

Then for every $X$, using Proposition \ref{prop:tensorconnection}, we compute
\[
0 = \conx_X F_i = \conx_X \sigma(G_i) = (\conx \sigma)(X, G_i) + \sigma(\conx_X G_i) = (\conx \sigma)(X, G_i)
\]
for each $i=1\cdots, n$. Since $X$ is arbitrary and $\{G_i\}$ is a frame we conclude that $\conx \sigma = 0$ and hence $\sigma$ is constant as required.
#+END_proof

We will call this unique Gauss map, the /parallel Riemannian Gauss map/.
...
\]
in $\RR^n$ where $\gaussmap{F}_U(\momentum)(x) = (\momentum_1(x), \cdotsc, \momentum_n(x))$ with $\gaussmap{F}_U$ is the parallel Riemannian Gauss map.
#+END_defn

The infinitesimal balance of momentum is equivalent to the balance of local momentum. The derivation is now formally the same as in Euclidean space as described for instance in \cite[Section 8.2]{MR960687}. See also \cite{MR2352963}.
