...
\end{split}
\]
where $\nor$ is a unit normal field to $\bdry{\Omega(x)}$, $\div_{\disc}$ is the divergence with respect to $\metric_{\disc}$ on $\disc$, and $\measure_{\bdry{\disc}}$ the induced ``surface'' measure on $\bdry{\Omega(x)}$.
\end{theorem}
