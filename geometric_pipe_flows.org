...
We need to be a little careful here. If we were integrating over the entire domain \(B\), we could use \(\pushforward{\phi} \partial_t = \vec{\velocity}\). This would be using the Lagrangian point of view which is the point of view adopted so far. However, here we want to fix \(x \in \domaincurve\) and integrate over the /changing/ cross section \(\xsec(x, t) = \body(t) \intersect \left(\{x\} \times \disc\right)\). Now we have the Eulerian point of view, where we do not follow a point along the flow.

#+BEGIN_LATEX
\note{Is the following correct? That is, can we parametrize \(\xsec(x,t)\) by say \(\psi_t : \Omega_0 \to \{x\} \times \disc\) so that \(\pushforward{\psi} \partial_t = \vec{\velocity}_{\disc}\)? Also we must relate the cross-sectional normal to the free surface normal. That is the free surface normal will have a component in the \(\partial_x\) direction also.}
#+END_LATEX
...
\begin{proof}
Keeping $x \in (0, \curvelength)$ fixed, we have that $\psi_{\star}\pd{t} = \discvec{\velocity}$.  Using the properties of the Lie derivative keeping mind that $\discvec{\velocity}$ is a time dependent vector field, we then compute
\[
\begin{split}
\pd{t} \int_{\xsec(t)} f \measure_{\disc} &= \int_{\xsec(0)} \pd{t} \psi_t^{\star} (f \measure_{\disc}) \\
&= \int_{\xsec(0)} \psi_t^{\star} \lie_{\discvec{\velocity}} (f \measure_{\disc}) \\
&= \int_{\xsec(0)} \psi_t^{\star} \left(\pd[f]{t} \measure_{\disc} + \lie_{\discvec{\velocity}} (f \measure_{\disc})\right) \\
&= \int_{\xsec(0)} \psi_t^{\star} \left(\pd[f]{t} \measure_{\disc} + \discvec{\velocity}(f) \measure_{\disc} + f \div_{\disc} \discvec{\velocity} \measure_{\disc} \right) \\
&= \int_{\xsec(0)} \psi_t^{\star} \left(\pd[f]{t} \measure_{\disc} + \div_{\disc}(f \discvec{\velocity}) \measure_{\disc}\right) \\
