...
#+BEGIN_LATEX
\note{Clean up definition of pipe and make it more concise. Derive equations with mimimal use of coordinates.}
#+END_LATEX
...
\label{thm:div_vec}
For any vector field $X = \xvec{X} + \discvec{X}$ and any \(\Omega \subset M\) with each \(\Omega(x)\) an open set with smooth enough boundary for Stokes' theorem to apply, we have
\[
\begin{split}
\int_{\Omega(x)} \div X \measure_{\disc}(x) &= \pd{x} (\xsecarea_{\Omega(x)} \avg{\xvec{X}})_{\Omega(x)} + \int_{\Omega(x)} \div_{\disc} \discvec{X} \measure_{\disc} \\
&= \pd{x} (\xsecarea_{\Omega(x)} \avg{\xvec{X}})_{\Omega(x)} + \int_{\bdry{\Omega(x)}} \ip{\discvec{X}}{\nor} \measure_{\bdry{\disc}}
\end{split}
\]
where $\nor$ is a unit normal field to $\bdry{\Omega(x)}$, $\div_{\disc}$ is the divergence with respect to $\metric_{\disc}$ on $\disc$, and $\measure_{\bdry{\disc}}$ the induced ``surface'' meausure on $\bdry{\Omega(x)}$.
\end{theorem}
...
\begin{remark}
If $\domain$ is a closed pipe, then $\bdry{\body}_{\is}$ truly is an impermeable surface.  If $\domain$ is an open channel, then $\domaincurve \times S^1$ has an impermeable part, the bottom of the channel and an open part.  The assumption of the model is that the fluid remains strictly inside the channel, so that $\bdry{\body}_{\is}$ is really the fluid boundary at the impermeable part of the channel.  For any solution (or more usually a numerical approximation), we need to explicitly check that the fluid has remained in the channel for the solution to be valid.
\end{remark}

** Balance Laws

We assume that $\body(t)$ comes equipped with a density $\density: \body(t) \to \RR$. The velocity is a time dependent vector field $\vec{\velocity}$ on $\body(t)$ for each $t\in\interval$. Recall that $\density$ and $\vec{\velocity}$ together satisfy the system of equations,
\begin{align*}
\pd{t} \density + \div_{\metric} (\density \vec{\velocity}) &= 0 \\
\pd{t} (\density \vec{\velocity}) + \div_{\metric} (\density\vec{\velocity} \tensor \vec{\velocity}) &= \div_{\metric} \cauchystress + \vec{\bodyforce}.
\end{align*}
...
\]
We will assume that the fluid surface is advected by the flow, so that $\norvec{\velocity} = \norvec{\fsvelocity}$. Thus we there is no loss of energy from the fluid into it's boundary, such as for example in blood flows where the artery wall deforms from the blood flow. That is, we consider only fluid flows in rigid pipes.

To determine the solution up to reparametrisation of the free boundary, it is is sufficient to specify $\norvec{\velocity}$ on $\relbdry[\body(t)]{M}$.  That is we specify
\[
\norvec{\velocity}(x, t) = U(x)
\]
...
\begin{align*}
\norvec{\velocity}(x, t) &= 0, & & x \in \bdry{\body(t)}_{\is} \\
\norvec{\velocity}(x, t) &= U_{\fs}(x), & & x \in \bdry{\body(t)}_{\fs}.
\end{align*}
Here, we assume that the fluid boundary is /advected/ by the flow, that is, $\norvec{\relvelocity} (x, t) = 0$. For a fluid flow in a rigid pipe, the fluid cannot pass through the impermeable boundary so that $\norvec{\velocity} = 0$. 

We leave the free surface unspecified for the moment, dealing with it when integrating over cross sections.

** Transport Theorem

The last ingredient we require is a formula for differentiating under the integral sign known in the literature as the (Reynold's) Transport Theorem.  

We need to be a little careful here, treating the cross section $\xsec(x,t)$ as a family of smooth domains in $(D, \metric_D(x))$ with velocity field $V_D(t)$ not in general equal to $\vec{\velocity}$ since points in $\Omega(x,t)$ come from different cross sections as $t$ varies.

The assumption that the fluid is advected by the flow says that on the free surface, the normal velocity of the fluid surface, $\ip{V}{\nor} = \ip{\vec{\velocity}}{\nor}$, the normal velocity of a fluid particle. The no-leak condition says that $\ip{\vec{velocity}}{\nor} = 0$ on the impermeable surface. Let us denote the unit normal to the cross section $\xsec(x,t)$ as $\xsecnor$. Then we can write $\nor = a \partial_x + b \xsecnor$ where $a^2 + b^2 = 1$.
...
#+BEGIN_LATEX
\note{Need to sort out the relationship between the normal component of the velocity of the fluid and the free surface.x}
#+END_LATEX
