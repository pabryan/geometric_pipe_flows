...
#+BEGIN_defn
A /motion/ of a body in $M$ is a smooth one parameter family of embeddings, $\phi_t(\cdot) = \phi(\cdot, t)$ with
\[
\phi : \body \times \timeinterval \to M
\]
for some $T>0$ and $\body$ a smooth manifold with piecewise smooth boundary $\bdry{\body}$.  That is, $\phi$ is continuous on $\clsr{\body} \times \timeinterval$, smooth on $\body \times \timeinterval$ and piecewise smooth on the smooth part, $\bdry{\body}_{\smooth} \times \timeinterval$, of the boundary of $B$.  For each $t\in\timeinterval$ the map $\phi_t(-) = \phi(-, t) : \clsr{\body} \to M$ is a homemorhpism with it's image and is a diffeomorphism of the interior of $\body$ with it's image.
...
The main point here is that we have a density $\density_t \metricmeasure{\metric}$ defined on $B_t$ where $\density_t: B_t \to \RR$ is smooth and $\metricmeasure{\metric} = \det\metric$ is the volume form on $M$ induced by the metric $\metric$. We want to compute the evolution of various quantities \emph{on $M$} such as the density, momentum (computed with respect to the density), acceleration etc. moving with the flow.  On $M$, such an infinitessimal change is measured by the Lie derivative $\lie_{\vec{\velocity}}$.  First, conservation of mass is expressed through the relation
\[
(\phi_t^{\star} (\density\metricmeasure{\metric})) (x) = (\density \metricmeasure{\metric})(\phi_t(x)) = (\density\metricmeasure{\metric}) (x, 0)
\]
for all $x\in \body$ and $t \in [0, T)$.  Since $\vec{\velocity} = \phi_{\star}\pd{t}$ is a time dependent family of vector fields on $M$ and $\metricmeasure{\metric}$ is independent of time, the infinitessemal conservation of mass can now be written
\[
