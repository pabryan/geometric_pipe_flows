...
#+BEGIN_prop
Let $U\subset M$ be a connected open set on which $\tangentbundle$ is trivial. Then there exists a unique Gauss map $\gaussmap{F}_U: \tangentbundle[U] \to U \times \RR^{n}$ such the frame $\{F_U^{-1}(e_i)\}$ is a parallel (with respect to the Levi-Civita connection), orthonormal field. Here $e_i$ denotes the section of $U \times \RR^n$, $x \mapsto (x, (0, \dotsc, 0, 1, 0, \dotsc, 0)$ with the $1$ in the $i$'th position.
