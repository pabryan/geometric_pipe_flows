...
#+BEGIN_defn
A /motion/ of a body in $(M, \metric)$ is a smooth one parameter family of embeddings, $\phi_t(\cdot) = \phi(\cdot, t)$ with
\[
...
\[
\measure_t = \density_t \phi_t^{\star} \metricmeasure{\metric}
\]
where $\density_t$ is a smooth family of positive smooth functions on $B$.
...
\[
\measure_t(x) = \measure_0(x)
\]
for all $x \in B$ and $t \in [0,T)$. That is, the density at $x$ is preserved along the flow. From the definition of $\measure_t$, we may write this as
\[
\density_t(x) (\phi_t^{\star} \metricmeasure{\metric})(x) = \density_0(x) \phi_0^{\star}(\metricmeasure{\metric})(x).
\]

Differentiating this expression with respect to $t$, the infinitessemal conservation of mass can now be written
\[
...
\[
0 = \left(\pd[\density]{t} + \div(\density\vec{\velocity})\right) \metricmeasure{\metric},
\]
which is in turn equivalent to
\[
0 = \pd[\density]{t} + \div(\density\vec{\velocity})
\]
