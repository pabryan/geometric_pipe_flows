...
#+BEGIN_LATEX
\note{Give balance laws}
#+END_LATEX

We assume that $\body(t)$ comes equipped with a density $\density: \body(t) \to \RR$. The velocity is a time dependent vector field $\vec{\velocity}$ on $\body(t)$ for each $t\in\interval$.  

#+BEGIN_LATEX
Recall Euler and Navier/Stokes here.
#+END_LATEX
